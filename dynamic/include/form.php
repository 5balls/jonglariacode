...
class Form
{
    ...
    protected $storeindb;
    protected $cfg;
    public function __construct(array $fd, $dn, $sbn, array $sd, $rd, $authuser = null, $storeindb = null){
        $this->cfg = new Config();
	$this->formdata = $fd;
        $this->dataname = $dn;
...
# Replace dataname by data in place and add to UserData:
    ...
    public function readdata(){
        if(is_null($this->storeindb)){
            ...
            # TODO: Check if database and database entry exists
            $data = new UserDataSQL($this->filename, $this->authuser);
            $data->fillFromFile();
            $content = array();
            foreach($this->storedata as $dindex => $dataset){
